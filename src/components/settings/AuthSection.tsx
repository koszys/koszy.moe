        ...
        <div className="bg-[#1c1d21] border border-[#52525b] rounded-xl p-4 md:p-6 shadow-lg">
            <div className="flex items-center justify-between mb-5">
                ...
                {user && user.identities && (
                    <div className="flex items-center gap-1.5 p-1 px-2.5 rounded-full border border-[#52525b] bg-[#18181b]/50 text-xs text-gray-400 font-medium">
            ...
            {user ? (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 md:p-5 bg-[#18181b] border border-[#3f3f46] rounded-lg">
                    <div className="flex items-center gap-3">
                        {user.avatar ? (
                            <img src={user.avatar} alt="Profile" referrerPolicy="no-referrer" className="w-10 h-10 rounded-full object-cover border border-[#52525b]" />
                        ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 md:p-5 bg-[#18181b] border border-[#3f3f46] rounded-lg">
                    ...
                    <button onClick={openModal} className="px-6 py-2 bg-blue-600 hover:bg-blue-500 border border-transparent hover:border-white rounded-lg text-sm font-bold transition-all w-full sm:w-auto">
                        Sign In
                    </button>
                </div>
            )}
